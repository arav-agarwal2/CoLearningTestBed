...
do
    for i in ${!I[@]}
    do
        python synthetic/${METHOD}.py --data-path /home/yuncheng/MultiBench/synthetic/SIMPLE_DATA_CLASS=32_DIM=5_STD=0.5.pickle --input-dim 5 --output-dim 32 --hidden-dim 64 --num-classes 32 --modalities ${I[i]} ${J[i]} ${K[i]} ${M[i]} > synthetic/32${METHOD}_$i.txt
    done
done