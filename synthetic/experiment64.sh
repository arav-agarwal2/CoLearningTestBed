...
do
    for i in ${!I[@]}
    do
        python synthetic/${METHOD}.py --data-path /home/yuncheng/MultiBench/synthetic/SIMPLE_DATA_CLASS=64_DIM=6_STD=0.5.pickle --input-dim 6 --output-dim 64 --hidden-dim 128 --num-classes 64 --modalities ${I[i]} ${J[i]} ${K[i]} > synthetic/64${METHOD}_$i.txt
    done
done