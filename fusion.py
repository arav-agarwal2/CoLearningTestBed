...
class MFN(nn.Module):
    ...
    def forward(self,x):
        x_l = x[2].transpose(1, 0).to(device)
        x_a = x[1].transpose(1, 0).to(device)
        x_v = x[0].transpose(1, 0).to(device)
        # shape: t x n x d
        n = x_l.shape[1]
        t = x_l.shape[0]
        self.h_l = torch.zeros(n, self.dh_l).to(device)
        self.h_a = torch.zeros(n, self.dh_a).to(device)
        self.h_v = torch.zeros(n, self.dh_v).to(device)
        self.c_l = torch.zeros(n, self.dh_l).to(device)
        self.c_a = torch.zeros(n, self.dh_a).to(device)
        self.c_v = torch.zeros(n, self.dh_v).to(device)
        self.mem = torch.zeros(n, self.mem_dim).to(device)
        all_h_ls = []
        all_h_as = []
        all_h_vs = []
        all_c_ls = []
