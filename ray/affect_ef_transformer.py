...
def affect_ef_transformer(config):
    ...
    saved_model = './{}_ef_transformer_{}.pt'.format(args.dataset, ''.join(list(map(str, modalities))))
...
saved_model = os.path.join(logdir, '{}_ef_transformer_{}.pt'.format(args.dataset, ''.join(list(map(str, modalities)))))
print("Using model: {}".format(saved_model))
model = torch.load(saved_model).cuda()

test(model, testdata, 'affect', is_packed=True, criterion=torch.nn.L1Loss(), task="posneg-classification", no_robust=True, modalities=modalities)
